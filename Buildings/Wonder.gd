...
func _process(delta):
	if !placed:
		...
		position = snapped
		# Upper bound x and y are -4 because this is a 5x5 building
		if position.x <= 0 - TILE_SIZE || position.x >= TILE_SIZE * (get_parent().rows-4) || position.y <= 0-TILE_SIZE || position.y >= TILE_SIZE * (get_parent().columns-4):
			self.hide()
			in_bounds = false
		else:
			self.show()
